...
var test_data = {};
fs.readFile('/var/www/newsgregator/node_modules/node-reportorial/lib/test.json', 'utf8', function (err,data) {
	console.log( 'GETTING TEST DATA', data );
	if (err) {
		return console.log(err);
	} else {
		test_data = JSON.parse( data ); 
	}
	console.log('test.json', test_data);
});
...
Public.prototype.listen = function( io_app ) {
	...
	io_app.get('/test', function( req, res, next ) {
		//zzz
		console.log( 'io_app.get /test', test_data );
