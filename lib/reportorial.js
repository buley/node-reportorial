...
Public.prototype.listen = function( io_app ) {
	...
	io_app.get('/:type/:id/:method?', function(req, res, next){
		...
		var data = req.body;
		console.log('request', type, id, method, req.body );
		console.log('TYPEOF2', typeof Reportorial.verify_incoming_message_is_subscription_confirmation_request( data ) );
		if( Reportorial.verify_incoming_message_is_subscription_confirmation_request( data ) ) {
			process_subscription_confirmation_request( data );
		} else if( Reportorial.verify_incoming_message_is_new_item_notification( data ) ) {
			process_new_item_notification( data );
		} else {
